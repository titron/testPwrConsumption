...
// D-40472 Duesseldorf
//============================================================================
// Purpose: UART A Application Access Interface
//
//Warranty Disclaimer
//
...
#define ASMN_MENULINE_HDR1  "UART A APPLICATION MENU\n"
...
bit
ApplUARTASendString( u08 MenuCom )
{
  bit           UARTStatus;
  ...
  if( SuccessFlag == UARTA_OK )
    ...
    SuccessFlag = UARTA_SendString( ( u08 )UnitNumber, 
                                    SendString, 
                                    ASMN_UARTMODECR );
  else {
...
ApplUARTLINEntry( u08 MenuCom )
{
  ...
  if( SuccessFlag == UARTA_OK )
    ...
    SuccessFlag = UARTA_LINApplTest( MenuCom, ( u08 )UnitNumber );
...
unsigned char ASMN_UARTAApplications( unsigned int MenuCom )
{
  ...
  while( ( UARTStatus  == ASMN_UARTTRANSFEROK ) && 
    ...
    case ASMN_MENUCODE_SEND:
      UARTStatus = ApplUARTASendString( MenuCom );
      break;
#endif
#ifdef ASMN_MENULINE_MEN2
    case ASMN_MENUCODE_LIN:
      UARTStatus = ApplUARTALINEntry( MenuCom );
      break;
#endif
