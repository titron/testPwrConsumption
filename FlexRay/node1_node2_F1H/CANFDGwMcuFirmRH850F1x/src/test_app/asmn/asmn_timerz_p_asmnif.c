...
// D-40472 Duesseldorf
//============================================================================
// Purpose: Timer Z Primitive Access Interface
//
//Warranty Disclaimer
//
...
/* Timer Z Driver Interface */
#include <map_timerz.h>
...
#define ASMN_MENULINE_HDR1  "TIMER Z TEST MONITOR\n"
#define ASMN_MENULINE_HDR2  "  (C) RL 07/2004    \n\n"
#define ASMN_MENULINE_MEN0  "0 -> Exit\n"

#if( TMZ_TYPE == QLNPUTMZV1 )
#define ASMN_MENULINE_MEN1  "1 -> Reset Timer Z\n"
#define ASMN_MENULINE_MEN2  "2 -> Configure Timer Z\n"
#define ASMN_MENULINE_MEN3  "3 -> Start/Stop Timer Z\n"
#define ASMN_MENULINE_MEN4  "4 -> Read Timer Z Timebase\n"
...
static u16 TMZ_InterruptCount_u16 = 0;

/*
 * interrupt functions
 */

void TestTimerZIRQNotifier( )
{
  TMZ_InterruptCount_u16++;
}
...
//=============================================================================
// FunctionName: TestTimerZConfig
// IN  : MenuCom:       Communication Port ID
// OUT : Return:        Communication status
// Description : 
// Get Arguments from user, execute the function above on the peripheral.
//=============================================================================
u08 TestTimerZConfig( u08 MenuCom )
{
  u08           UARTStatus;
  u08           ScanState;
  bit           SuccessFlag = TMZ_OK;
  int           UnitNumber  = -1;
  int           Modulus;
  int           ClockSelection;
  u16           PModulus_u16;
  u08           PClockSelection_u08;
  ...
  UARTStatus = ASMN_UARTRECEIVEINT( MenuCom, ASMN_MENUINPUT_BUFLEN,
                				    ...
                				    &ScanState );
  if( UARTStatus != ASMN_UARTTRANSFEROK ) return( UARTStatus );
  if( ScanState  != ASMN_MENUCODE_SCANOK ) SuccessFlag = TMZ_ERROR;

  ScanState = TMZ_GetConfiguration( ( u08 )UnitNumber,
                                    &PModulus_u16,
                                    &PClockSelection_u08 );

  if( ScanState  != ASMN_MENUCODE_SCANOK ) 
  {
    SuccessFlag = TMZ_ERROR;
  }
  else 
  {
    Modulus        = ( int )PModulus_u16;
    ClockSelection = ( int )PClockSelection_u08;
  }
  ...
  if( ( u08 )ClockSelection == PClockSelection_u08 ) 
    ClockSelection = TMZ_CLOCK_KEEP;

  if( SuccessFlag == TMZ_OK )
  {
    SuccessFlag = TMZ_SetConfiguration( ( u08 )UnitNumber,
                                        ( u16 )Modulus,
                                        ( u08 )ClockSelection );
  }
  else {
...
//=============================================================================
// FunctionName: TestTimerZReset
// IN  : MenuCom:       Communication Port ID
// OUT : Return:        Communication status
// Description : 
// Get Arguments from user, execute the function above on the peripheral.
//=============================================================================
u08 TestTimerZReset( u08 MenuCom )
{
  u08           UARTStatus;
  u08           ScanState;
  bit           SuccessFlag = TMZ_OK;
  ...
  if( SuccessFlag == TMZ_OK )
    SuccessFlag = TMZ_Reset( ( u08 )UnitNumber );
  else {
...
//=============================================================================
// FunctionName: TestTimerZStartStop
// IN  : MenuCom:       Communication Port ID
// OUT : Return:        Communication status
// Description : 
// Get Arguments from user, execute the function above on the peripheral.
//=============================================================================
u08 TestTimerZStartStop( u08 MenuCom )
{
  u08           UARTStatus;
  u08           ScanState;
  bit           SuccessFlag = TMZ_OK;
  ...
  if( SuccessFlag == TMZ_OK )
    SuccessFlag = TMZ_StartStop( ( u08 )UnitNumber,
                                 ( u08 )Enable );
  else {
...
//=============================================================================
// FunctionName: TestTimerZReadTB
// IN  : MenuCom:       Communication Port ID
// OUT : Return:        Communication status
// Description : 
// Get Arguments from user, execute the function above on the peripheral.
//=============================================================================
u08 TestTimerZReadTB( u08 MenuCom )
{
  u08           UARTStatus;
  u08           ScanState;
  bit           SuccessFlag = TMZ_OK;
  ...
  if( SuccessFlag == TMZ_OK )
    SuccessFlag = TMZ_Read( ( u08 )UnitNumber, 
                            ( pu16 )( &ReadValue ) );
  else {
...
u08 TestTimerZInterrupt( u08 MenuCom )
{
  ...
  if( SuccessFlag == TMZ_OK ) {
    ...
    TMZ_InterruptCount_u16 = 0;

    SuccessFlag = TMZ_CreateInterrupt( ( u08 )UnitNumber,
                                       ( u08 )IRQSource,
                                       ( u08 )IRQLevel,
                                       TestTimerZIRQNotifier );
  }
  ...
  if( SuccessFlag == TMZ_OK ) {
    ...
    do {
      ...
      if( TMZ_InterruptCount_u16 >= EveryCount ) {
...
//=============================================================================
// FunctionName: ASMN_TimerZFunctions
// IN  : MenuCom:       Communication Port ID
// OUT : Return:        Communication status
// Description : 
// Decode and execute Timer C Primitive Menu Functions.
//=============================================================================
unsigned char ASMN_TimerZFunctions( u08 MenuCom )
{
  ...
  while( ( UARTStatus  == ASMN_UARTTRANSFEROK ) && 
    ...
    case ASMN_MENUCODE_CNF:
      UARTStatus = TestTimerZConfig( MenuCom );
      break;

    case ASMN_MENUCODE_SSTOP:
      UARTStatus = TestTimerZStartStop( MenuCom );
      break;
      
    case ASMN_MENUCODE_RESET:
      UARTStatus = TestTimerZReset( MenuCom );
      break;
      
    case ASMN_MENUCODE_RDTB:
      UARTStatus = TestTimerZReadTB( MenuCom );
      break;
      
    case ASMN_MENUCODE_IRQ:
      UARTStatus = TestTimerZInterrupt( MenuCom );
      break;
