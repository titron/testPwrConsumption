...
#define ASMN_MENULINE_HDR1  "MTTCAN APPLICATION MENU\n"
#define ASMN_MENULINE_HDR2  "  (C) RL 04/2016    \n\n"
#define ASMN_MENULINE_MEN0  " 0 -> Exit\n"

#if( MTTCAN_TYPE == MTTCAN_R301 )
#define ASMN_MENULINE_MEN1  " 1 -> Reset\n"
#define ASMN_MENULINE_MEN2  " 2 -> Basic Test: Sending and Receiving\n"
#define ASMN_MENULINE_MEN3  " 3 -> Transmit Test\n"
...
#define ASMN_MENUINPUT_WAIT      "\n Enter Wait on ACK [0|1] "
...
bit
ApplMTTCANReset( u08 MenuCom )
{
  bit           UARTStatus;
  unsigned char ScanState       = ASMN_MENUCODE_SCANOK;
  bit           SuccessFlag     = MTTCAN_OK;
  int           Unit_1          = 0;
  ...
  UARTStatus = ASMN_UARTRECEIVEINT( MenuCom, ASMN_MENUINPUT_BUFLEN,
				    ...
				    &ScanState );
  if( UARTStatus != ASMN_UARTTRANSFEROK ) return( UARTStatus );
  if( ScanState  != ASMN_MENUCODE_SCANOK ) SuccessFlag = MTTCAN_ERROR;

  if( SuccessFlag == MTTCAN_OK ) {

    SuccessFlag = MTTCAN_Reset( ( u08 )Unit_1 );
  }
  else {
...
bit
ApplMTTCANBasicTest( u08 MenuCom )
{
  bit           UARTStatus;
  unsigned char ScanState       = ASMN_MENUCODE_SCANOK;
  bit           SuccessFlag     = MTTCAN_OK;
  ...
  if( SuccessFlag == MTTCAN_OK ) {
    ...
    SuccessFlag = MTTCAN_BasicTest( ( u08 )Unit_1, 
                                  ( u08 )Unit_2 );
  }
  else {
...
bit
ApplMTTCANTransmitTest( u08 MenuCom )
{
  bit           UARTStatus;
  unsigned char ScanState       = ASMN_MENUCODE_SCANOK;
  bit           SuccessFlag     = MTTCAN_OK;
  ...
  if( SuccessFlag == MTTCAN_OK ) {
    ...
    SuccessFlag = MTTCAN_TransmitTest( ( u08 )Unit_1,
                                       ( u08 )WaitOnAck );
  }
  else {
...
bit
ApplMTTCANGatewayTest( u08 MenuCom )
{
  bit           UARTStatus;
  unsigned char ScanState       = ASMN_MENUCODE_SCANOK;
  bit           SuccessFlag     = MTTCAN_OK;
  ...
  if( SuccessFlag == MTTCAN_OK ) {
    ...
    SuccessFlag = MTTCAN_Gateway( ( u08 )Units );
  }
  else {
...
bit ASMN_MTTCANApplications( u08 MenuCom )
{
  ...
  while( ( UARTStatus  == ASMN_UARTTRANSFEROK ) && 
    ...
    case ASMN_MENUCODE_TEST1:
      UARTStatus = ApplMTTCANReset( MenuCom );
      break;
#endif

#ifdef ASMN_MENUCODE_TEST2
    case ASMN_MENUCODE_TEST2:
      UARTStatus = ApplMTTCANBasicTest( MenuCom );
      break;
#endif

#ifdef ASMN_MENUCODE_TEST3
    case ASMN_MENUCODE_TEST3:
      UARTStatus = ApplMTTCANTransmitTest( MenuCom );
      break;
#endif

#ifdef ASMN_MENUCODE_TEST4
    case ASMN_MENUCODE_TEST4:
      UARTStatus = ApplMTTCANGatewayTest( MenuCom );
      break;
#endif
