...
/* Used for configuring the RIIC code */
    #include "riic_config/r_riic_rh850f1x_config.h"
...
/*----------------------------------------------------------------------------*/
    #define RIIC_GEN_START_CON      ((uint32_t)(0x01))   /* Generate start condition */
    #define RIIC_GEN_STOP_CON       ((uint32_t)(0x02))   /* Generate stop condition */
    #define RIIC_GEN_RESTART_CON    ((uint32_t)(0x04))   /* Generate restart condition */
    #define RIIC_GEN_SDA_HI_Z       ((uint32_t)(0x08))   /* Generate nack output */
    #define RIIC_GEN_SCL_ONESHOT    ((uint32_t)(0x10))   /* Geenrate one-shot scl clock output */
    #define RIIC_GEN_RESET          ((uint32_t)(0x20))   /* Reset riic module */
...
/*----------------------------------------------------------------------------*/
typedef uint32_t riic_ch_dev_status_t;
...
typedef volatile struct
{
    ...
    uint32_t              ch_no; /* Channel No. */
    riic_callback        callbackfunc; /* Callback function */
    uint32_t             cnt2nd; /* 2nd Data Counter */
    uint32_t             cnt1st; /* 1st Data Counter */
    uint32_t            * p_data2nd; /* Pointer for 2nd Data buffer */
    uint32_t            * p_data1st; /* Pointer for 1st Data buffer */
    uint32_t            * p_slv_adr; /* Pointer for Slave address buffer */
} riic_info_t;
